...
#define DEFINE_RB_CLASS_FOR_STRUCT(type) \
  ...
  \
  const rb_data_type_t rb_##type##_type = { \
  ...
  static VALUE rb_##type##_alloc(VALUE self) { \
    type##_t *obj; \
    return TypedData_Make_Struct(self, type##_t, &rb_##type##_type, obj); \
  } \
  \
  static VALUE rb_##type##_initialize(int argc, VALUE* argv, VALUE self) { \
    ...
    \
    type##_t *wrapper; \
    TypedData_Get_Struct(self, type##_t, &rb_##type##_type, wrapper); \
    \
...
#define DEFINE_UINT128_ACCESSORS(type, field_name) \
  static VALUE rb_##type##_get_##field_name(VALUE self) { \
    type##_t *obj; \
    TypedData_Get_Struct(self, type##_t, &rb_##type##_type, obj); \
